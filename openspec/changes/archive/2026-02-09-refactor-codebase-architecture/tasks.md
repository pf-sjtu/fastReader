...
### 2.5 统一类型定义 [P1] - **已完成**
- [x] 创建 `types/chapter.ts`
- [x] 创建 `types/metadata.ts`
- [x] 创建 `types/ai.ts`
- [x] 创建 `types/batch.ts`
- [x] 创建 `types/index.ts` 统一导出
- [x] 更新 `services/ai/types.ts` 为兼容壳（从 @/types 重新导出）
- **完成日期**: 2026-02-08
...
### 3.1 PDF章节并行提取 [P1] - **已完成**
- [x] 使用 `ConcurrencyLimiter` 类（已在utils/async.ts实现）
- [x] 修改 `pdfProcessor.ts` 使用并行提取
- [x] 控制并发数为3
- **完成日期**: 2026-02-08
- **依赖**: 1.1完成
- **并行**: 可与2.x部分并行

### 3.2 EPUB章节并行提取 [P1] - **已完成**
- [x] 修改 `epubProcessor.ts` 使用并行提取
- [x] 使用 ConcurrencyLimiter 控制并发数为3
- [x] 确保 Section 正确卸载
- **完成日期**: 2026-02-08
- **依赖**: 1.3完成
- **并行**: 可与3.1并行
...
### 3.4 实现请求去重机制 [P2] - **已完成**
- [x] 在 `aiService.ts` 中添加飞行中请求缓存 (pendingRequests Map)
- [x] 实现请求key生成 (generateRequestKey)
- [x] 复用进行中的相同请求
- **完成日期**: 2026-02-08
- **依赖**: 2.2完成
- **并行**: 可与3.3并行
